...
def init_population(pop_size):
    ...
    mask = np.linalg.norm(combined - 420.9687, axis=1) < 50
    while np.any(mask):
        combined[mask] = np.random.uniform(LOWER_BOUND, UPPER_BOUND, (np.sum(mask), DIM))
        mask = np.linalg.norm(combined - 420.9687, axis=1) < 50
...
def run_ga():
    ...
    for gen in range(MAX_GEN):
        ...
        # 对立学习应用在新种群
        opp_pop = LOWER_BOUND + UPPER_BOUND - new_pop
        combined = np.vstack((new_pop, opp_pop))
        fitness = schwefel(combined)
        best_indices = np.argsort(fitness)[:POP_SIZE]
...
ani.save("schwefel_ga_obl_animation.gif", writer="pillow", fps=10)
# ani.save("schwefel_ga_obl_animation.mp4", writer="ffmpeg", fps=10)
